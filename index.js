...
client.on("guildCreate", guild => {
    ...
    if (role == null || role == undefined) {
        guild.createRole({
            ...
            mentionable: !0const Discord = require("discord.js");
const client = new Discord.Client();
var fs = require('fs');
var config = {};
const prefix = "v.";
...
client.on("message", (message) => {
    ...
    if (botExec(message.member)) {
        ...
        if (command === "kick") {
            ...
            await member.kick(reason).catch(error => message.reply(`the user could not be kicked because of: ${error}`));
            message.reply(`${member.user.tag} has been kicked by ${message.author.tag} due to the following reason: \n \`\`\`${reason}\`\`\``)
        }
        if (command === "say") {
            message.delete();
            message.channel.send(args.join(" "))
    ...
    if (member.user.id === "311534497403371521") {
        if (command === "setgame") {
            var game = message.channel.send(args.join(" "));

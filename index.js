...
client.on('guildMemberAdd', member => {
  ...
  if (member.guild.id === "369492433060364300") {
    ...
    const roleIntro = member.guild.roles.find('name', 'Intro');
    member.addRole(roleIntro);
  } else if (member.guild.id === "383287520118702100") {
    member.send("Welcome to the official Pokémon Equality server! Please not that you cannot talk for the first 10 minutes. This is for several security reasons. In the mean time, please check out the FAQ and Rules channels.");
  } else {
...
client.on("message", (message) => {
  ...
  if (botExec(message.member)) {
    ...
    if (command === "clear") {
      ...
      if (index > 100)
        return message.channel.send('Only 100 messages can be cleared at a time. Please specify a smaller amount of message.');
      if (index < 2)
